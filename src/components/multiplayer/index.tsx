      ...
      {numberOfPlayers !== 1 && (
        <div className="flex justify-center gap-[1.5rem] w-full">
          {players.slice(0, numberOfPlayers).map((player) => (
            <div
              key={player.id}
              className="md:max-w-[10rem] relative max-w-[4rem] w-full"
            >
              <span
                className={clsx(
                  turn === player.id ? "flex" : "hidden",
                  "md:translate-x-[4.5rem] w-[1rem] h-[1rem] bg-[#FDA214] rotate-45 absolute translate-x-[1.5rem] translate-y-[-0.4rem]"
                )}
              ></span>
              <div
                className={clsx(
                  turn === player.id ? "bg-[#FDA214]" : "bg-[#DFE7EC]",
                  "md:max-w-[10rem] md:justify-between md:items-start md:px-[1.5rem] md:py-[1.1rem] relative max-w-[4rem] flex flex-col items-center w-full p-[0.65rem] rounded-lg"
                )}
              >
                <h3
                  className={clsx(
                ...
                <h3
                  className={clsx(
                    turn === player.id ? "text-[#FCFCFC]" : "text-[#7191A5]",
                    "md:flex hidden text-[1.15rem] leading-[1.2rem] font-[700] mb-[0.5rem]"
                  )}
                >
                  Player {player.name}
                </h3>
